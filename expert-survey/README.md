...
최초 30문항 완료 후에는 기존 알고리즘 배정과 다르게 답한 항목 중 확신도가 가장 높은 문항을 최대 3개까지 다시 제시한다. 이 단계에서는 알고리즘 선택과 본인의 원래 답변을 함께 보여주고, 원래 답변을 유지하거나 알고리즘 선택으로 수정할 기회를 제공한다. Markdown 응답에는 원래 선택과 재검토 후 선택을 별도 표로 분리해 저장한다.
