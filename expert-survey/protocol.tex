...
\begin{itemize}
\item L4 카드 182개, L3 위험군 24개, 독립 전문가 9명 이상 모집
\item 전체 카드의 최소 3회 평가를 위해 최소 19개 완주 응답 목표
...
\item 알고리즘 배정과 다른 고확신 응답을 최대 3개까지 재검토한 뒤 구조화된 Markdown 응답 저장
\end{enumerate}
...
응답은 브라우저 localStorage에 자동 임시저장하고 완료 시 구조화된 Markdown으로 변환해 공개 GitHub 응답 저장소에 자동 보존한다. 설문 시작·종료 시각, 총 소요시간과 국가 수준의 접속지역은 응답과 분리된 비공개 GitHub 저장소에 1년간 보존한다. 원본 IP와 더 세부적인 위치·접속·기기 정보는 저장하지 않으며 공개 응답에는 시간 및 국가 정보를 포함하지 않는다. 제한시간은 없다.
...
카드별 기존 L3 다수선택률과 완전일치율, family별 선택률, macro-average와 worst-family 결과, 후보쌍별 선택률, 확신도별 결과 및 margin--인간 선택 관계를 보고한다. 570개 계획 판단을 독립 표본으로 간주하지 않는다. 9--18개 완주 응답은 전체 카드 검증을 위한 중간자료로만 취급한다.
...
이중언어 계획서(Markdown, LaTeX, PDF), GitHub Pages 설문, 24개 L3 및 182개 L4 고정 데이터 스냅샷, A01--A19 배정표, Markdown 응답과 데이터 사전을 제공한다.
\end{document}
