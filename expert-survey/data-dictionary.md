...
완료 응답은 YAML front matter와 Markdown 표, JSON 객체, CSV 행으로 함께 저장된다.
...
| `Reference assignment` | L3 ID | 사전 지정 기준 배정 / Pre-specified reference assignment |
...
| `primary_endpoint` | string | 주 분석은 원래 응답의 기준 배정 agreement만 사용 |
| `minimum_completed_responses` | integer | 계획된 최소 완주 응답 수; 현재 20 |
...
최초 선택과 재검토 후 선택은 별도 표와 별도 JSON/CSV 필드에 분리해 저장한다. 주 분석은 최초 응답만 사용한다. 재검토 문항은 사전 지정 기준 배정과 다르게 답한 문항 중 확신도가 가장 높은 항목을 최대 3개까지 제시하며, 재검토 결과는 고확신 불일치 판단의 안정성과 원래 전문가 답변에서 기준 배정으로 전환한 switch rate를 보는 보조 분석으로만 사용한다.

Original choices and revised choices are retained in separate Markdown tables and separate JSON/CSV fields. The primary analysis uses only original responses. Reconsideration items are limited to at most three highest-confidence cases where the respondent's original choice differs from the pre-specified reference assignment, and are used only for secondary analysis of disagreement stability and switch rate.

Planned summaries are separated at card, family, and respondent levels. Card-level agreement should be reported with Wilson or bootstrap confidence intervals. The forced-choice pairwise format excludes unknown or unmappable judgments by design.
