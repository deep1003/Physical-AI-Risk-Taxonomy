...
The published taxonomy is a bilingual L4 risk-card view for Responsible AI Risk Taxonomy v2.0, organized as `L1 Physical AI Risks` > `L2 Categories` > `L3 Sub-categories` > `L4 Risk Cards`. Each card contains:
...
L1 is the Physical AI Risks slice. L2 uses `P2`, `I2`, and `S2` to show that System Safety, Interaction Safety, and Societal Safety are second-level categories under Physical AI Risks. L3 uses the same domain prefix with level number `3`, such as `P3.1`, `I3.4`, or `S3.9`. L4 risks remain identified by their card IDs.
