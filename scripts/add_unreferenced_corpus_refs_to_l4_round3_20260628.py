...
OUT_DIR = ROOT / "AI_Topic_Space.github.io/data/unreferenced_corpus_l4_reference_addition_round3_20260628"
...
PREVIOUS_ADDITIONS = [
    ...
    ROOT / "AI_Topic_Space.github.io/data/unreferenced_corpus_l4_reference_addition_round2_20260628/applied_unreferenced_corpus_l4_reference_additions.csv",
]

BLOCKLIST_FILES = [
    ROOT / "AI_Topic_Space.github.io/data/unreferenced_corpus_l4_reference_addition_20260627/removed_unreferenced_corpus_l4_reference_additions_after_quality_review.csv",
    ROOT / "AI_Topic_Space.github.io/data/l4_reference_relevance_audit_20260628/removed_low_relevance_references_applied.csv",
    ROOT / "AI_Topic_Space.github.io/data/unreferenced_corpus_l4_reference_addition_round2_20260628/removed_round2_after_quality_review.csv",
]
...
def referenced_keys(soup: BeautifulSoup) -> tuple[set[str], set[str], set[str]]:
    ...
    for p in BLOCKLIST_FILES:
        if not p.exists() or not p.stat().st_size:
            continue
        df = pd.read_csv(p)
        for _, row in df.iterrows():
            for title_col in ["title", "reference_title_full", "removed_reference_title_audit", "removed_reference_title_html"]:
                title = norm_text(row.get(title_col, ""))
                if title:
                    titles.add(norm_key(title.replace("+ ", "")))
            for url_col in ["url", "reference_url", "checked_url"]:
                u = norm_text(row.get(url_col, ""))
                if u:
                    urls.add(u.lower().rstrip("/"))
                    d = doi_key(u)
                    if d:
                        dois.add(d)
            d = doi_key(row.get("doi", ""))
            if d:
                dois.add(d)
...
def match_score(card: Card, row: pd.Series) -> tuple[float, set[str]]:
    ...
    if card.direct_terms and not direct_hits:
        return 0.0, doc_tags
    # Round 2 is intentionally stricter: keep only direct, card-specific overlaps.
    if card.direct_terms and len(direct_hits) < 2:
        strong_singletons = {
            "jailbreak", "spoofing", "spoof", "hijacking", "hijack", "teleoperation",
            "privacy", "surveillance", "fallback", "latency", "localization",
            "collision", "grip", "tactile", "drone", "airspace", "rehabilitation",
        }
        if not (direct_hits & strong_singletons and len(overlap) >= 2):
            return 0.0, doc_tags
...
def main() -> None:
    ...
    # Check a buffer so inaccessible links can be replaced while still producing 400 records.
    buffer = scored_df.head(900).to_dict("records")
    checked = []
    ...
    selected = accessible.head(400).copy()
    selected.insert(0, "rank", range(1, len(selected) + 1))

    selected.to_csv(OUT_DIR / "unreferenced_corpus_top400_access_checked.csv", index=False)
    ...
    for _, row in matches_df.iterrows():
        ...
        if doc_use_count.get(str(doc_id), 0) >= 2:
            skipped.append({**row.to_dict(), "skip_reason": "document_reuse_limit_2"})
            continue
    ...
    report = {
        ...
        "selected_top400_count": int(len(selected)),
    ...
    lines = [
        ...
        "",
        f"- Selected access-checked Top 400: {len(selected)}",
